...
from old.config import TOKEN

from keyboards.keyboards import set_up_path, testing_managing, path_periods_testing_help
from old.config.states import PytestPath, Notification, Failure

import logging
import asyncio

from database_config import create_database
from database_config.database_class import SQLiteDatabase
...
Enjoy 🍿
    ...
    await message.answer('To set the path click one of the buttons below🔽', reply_markup=path_periods_testing_help)
    ...
    async with state.proxy() as data:
        ...
        if not path.exists(pytest_path):
            await message.answer(
                ...
                reply_markup=path_periods_testing_help)
            ...
            await message.answer(
                'Path successfully set...', reply_markup=path_periods_testing_help)
...
async def stop_testing(message: types.Message):
    if not db.user_exists(message.from_user.id):
        ...
        await message.answer('Testing is being stopped...', reply_markup=path_periods_testing_help)
...
async def notification_period(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        ...
        try:
            time = int(time)
            if time < SLEEP_TIME or time > 20000:

                # if time input is integer but less than update period
                text = f'Failure period is not updated. Please, input the integer in range _[{SLEEP_TIME}; 20000]_'
                ...
                await message.answer(
                    'Notification period is updated...', reply_markup=path_periods_testing_help)
    ...
    async with state.proxy() as data:
        ...
        try:
            ...
            if time < SLEEP_TIME or time > 20000:
                ...
                await message.answer(
                    text=text,
                    reply_markup=path_periods_testing_help, parse_mode='Markdown')
                dp.current_state(user=message.from_user.id)
                await state.reset_state()
                ...
                await message.answer(
                    'Failure period is updated...', reply_markup=path_periods_testing_help)
...
async def main():
    ...
    try:
        ...
        loop.create_task(check_tests(SLEEP_TIME))
